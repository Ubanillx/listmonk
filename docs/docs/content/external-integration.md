...
Add `customers:read` and `campaigns:recipients` only when recipient-level data is required. Add media scopes only when the automation uploads or reads media.
