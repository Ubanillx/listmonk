...
SELECT COUNT(*) OVER () AS total,
    ...
    bounces.meta,
    bounces.created_at,
    COALESCE(bounces.customer_id, 0) AS customer_id,
    COALESCE(bounces.pool_contact_id, 0) AS pool_contact_id,
    COALESCE(bounces.source_pool_id, 0) AS source_pool_id,
    COALESCE(bounces.source_allocation_id, 0) AS source_allocation_id,
    bounces.source_organization_id,
    COALESCE(customers.uuid::text, pool_contacts.uuid::text, '') AS customer_uuid,
    COALESCE(customers.email, pool_contacts.email, '') AS email,
    COALESCE(customers.status::text, pool_contacts.status, '') as customer_status,
    (
