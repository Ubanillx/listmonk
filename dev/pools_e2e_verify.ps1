...
$replyMailboxID = [int](DbScalar "SELECT reply_mailbox_id FROM pool_segments WHERE id=$segmentID")
...
$segments = Api 'GET' "/api/pools/$poolID/segments" $manager 1 $null
$segmentRows = @($segments.Json.data)
Check 'organization can inspect its secondary list' ($segments.Status -eq 200 -and $segmentRows.Count -eq 1)
Check 'internal reply mailbox remains unmasked' ($segments.Raw.Contains('pool-replies@example.test'))
...
# permission. Preview succeeds because the unique secondary list and its
...
if ($campaignID -gt 0) {
  try {
    $attached = Api 'POST' "/api/campaigns/$campaignID/pools" $manager 1 ([pscustomobject]@{ pool_id = $poolID; pool_segment_id = $segmentID; organization_id = 1 })
    Check 'same campaign can add explicit secondary audience' ($attached.Status -eq 200)
    $poolRecipientCount = [int](DbScalar "SELECT count(*) FROM campaign_pool_recipients WHERE campaign_id=$campaignID")
    Check 'first-level and explicit secondary expansion deduplicate by pool contact' ($poolRecipientCount -eq 3)
    ...
    Check 'campaign metadata retains first-level and explicit secondary sources' ($campaignPools.Count -eq 2)
    $updated = Api 'PUT' "/api/campaigns/$campaignID" $manager 1 $campaignBody
    Check 'draft campaign can replace explicit secondary audience' ($updated.Status -eq 200)
    ...
    Check 'draft audience replacement removes stale secondary metadata' (@($updatedPoolsValue).Count -eq 1)
...
$mailboxCleared = Api 'PUT' "/api/pool-segments/$segmentID/reply-mailbox" $manager 1 ([pscustomobject]@{ reply_mailbox_id = $null })
$blockedID = 0
try {
  Check 'segment reply mailbox can be cleared for validation' ($mailboxCleared.Status -eq 200)
  ...
  if ($blockedID -gt 0) {
    try {
      ...
      Check 'preview is blocked when secondary reply mailbox is missing' ($blockedPreview.Status -eq 400)
    } finally {
  ...
  $mailboxRestored = Api 'PUT' "/api/pool-segments/$segmentID/reply-mailbox" $manager 1 ([pscustomobject]@{ reply_mailbox_id = $replyMailboxID })
  Check 'segment reply mailbox is restored after validation' ($mailboxRestored.Status -eq 200)
}
