...
Customer resources are exposed under `/api/customers`. Use the import API for
bulk ingestion and the customer endpoints for individual records. Customer
codes are business fields and are not unique identifiers.

See the [Import API](import.md) and the [API introduction](apis.md) for
...
| PUT    | [/api/customers/customer-lists](#put-apicustomerscustomer-lists)                            | Modify customer customer_list memberships.            |
| PUT    | [/api/customers/customer-lists/{customer_id}](#put-apicustomerscustomer-listscustomer_id) | Modify one customer's customer_list memberships. |
...
Retrieve a specific customer's activity: campaign views, link clicks, and reply-AI events.

> **Note:** Requires the `customers:get` or `customers:get_all` grant (`cmd/workspace_permissions.go:376`).
...
```shell
curl -u 'api_username:access_token' 'http://localhost:9000/api/customers/1/activity'
```
        ...
        "campaign_views": [
            {
                ...
                "subject": "Welcome!",
                "view_count": 3,
                "last_viewed_at": "2024-08-22T09:05:12.862877Z"
            }
        ],
        "link_clicks": [
            {
                ...
                "campaign_id": 2,
                "campaign_uuid": "eb420c55-4cfb-4972-92ba-c93c34ba475d",
                "campaign_name": "Welcome to listmonk",
                "campaign_subject": "Welcome!",
                "click_count": 1,
                "last_clicked_at": "2024-08-22T09:05:14.12Z"
            }
        ],
        "reply_ai_events": []
    }
...
> **Note:** Read access is resolved in the active workspace (`X-Listmonk-Organization-ID`). Only activity belonging to the customer's own owner and organization is returned.
...
#### POST /api/customers/{customer_id}/optin
...
`has_optin` is `true` when the subscription included a double opt-in customer list, so the customer has to confirm the subscription from the opt-in e-mail before receiving campaign messages.
...
#### PUT /api/customers/customer-lists/{customer_id}

Modify the customer_list memberships of a single customer. This is the single-customer form of [PUT /api/customers/customer-lists](#put-apicustomerscustomer-lists).

##### Parameters

| Name                     | Type       | Required           | Description                                                       |
| :----------------------- | :--------- | :----------------- | :---------------------------------------------------------------- |
| customer_id              | number     | Yes                | ID of the customer to modify.                                     |
| target_customer_list_ids | number[]   | Yes                | Array of customer_list IDs to be modified.                        |
| action                   | string     | Yes                | Action to be applied: `add`, `remove`, or `unsubscribe`.          |
| status                   | string     | Required for `add` | Subscription status: `confirmed`, `unconfirmed`, or `unsubscribed`. |

The customer ID in the URI takes precedence; the `ids` body field is ignored for this route.

##### Example Request

```shell
curl -u 'api_username:access_token' -X PUT 'http://localhost:9000/api/customers/customer-lists/1' \
-H 'Content-Type: application/json' \
--data-raw '{"action": "add", "target_customer_list_ids": [4, 5, 6], "status": "confirmed"}'
```
...
> **Note:** Requires the `customers:membership_manage` grant and manage access to the target customer_lists.
