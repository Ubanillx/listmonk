...
- 公海和二级列表的客户计数及查看入口使用 `pool_members`/`pool_segment_members` 专用查询；管理端不会把 `pool_contacts` 伪装成普通 `customers`，也不会让普通客户批量操作或导出路径接触公海数据。一级列表对组织用户只返回本组织已分配的安全 DTO，最高管理员可查看一级/二级完整记录。
