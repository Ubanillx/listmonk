<template>
  <section class="personal-api-keys mt-6">
    <div class="level api-key-header mb-5">
      <h2 class="title is-5 mb-1">API Keys</h2>
      <b-button type="is-primary" icon-left="plus" @click="openCreate">New API key</b-button>
    </div>
    ...
    <div v-else class="table-container api-key-table-wrap">
      <table class="table is-fullwidth is-hoverable">
        <thead>
          <tr>
            <th>Name</th><th>Workspace</th><th>Scopes</th><th>Expires</th><th>Last used</th><th>Status</th>
        ...
        <tbody>
          <tr v-for="key in keys" :key="key.id">
            ...
            <td class="has-text-right">
              <b-button size="is-small" type="is-text" icon-left="pencil-outline" title="Edit API key" :disabled="!isActive(key)" @click="openEdit(key)" />
              <b-button size="is-small" type="is-text" icon-left="refresh" title="Rotate API key" :disabled="!isActive(key)" @click="openRotate(key)" />
              <b-button size="is-small" type="is-text" icon-left="trash-can-outline" title="Revoke API key" :disabled="!isActive(key)" @click="revoke(key)" />
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <b-modal :active.sync="showEditor" has-modal-card :can-cancel="!saving">
      <form class="modal-card" @submit.prevent="save">
        <header class="modal-card-head"><p class="modal-card-title">{{ editingID ? 'Edit API key' : 'New API key' }}</p></header>
        <section class="modal-card-body">
          <b-field label="Name" label-position="on-border"><b-input v-model="form.name" maxlength="200" required autofocus /></b-field>
          <b-field v-if="!editingID" label="Workspace" label-position="on-border">
          ...
          <b-field label="Expires" label-position="on-border">
          ...
          <div class="api-key-scopes">
            <div class="level is-mobile mb-2"><p class="label mb-0">Permissions</p><b-checkbox v-model="allScopesSelected">All</b-checkbox></div>
            <div class="api-key-scope-grid">
              <b-checkbox v-for="scope in scopeOptions" :key="scope.id" v-model="form.scopes" :native-value="scope.id">{{ scope.label }}</b-checkbox>
            </div>
          </div>
        </section>
    ...
    <b-modal :active.sync="showRotation" has-modal-card :can-cancel="!rotating">
      <form class="modal-card" @submit.prevent="rotate">
        <header class="modal-card-head"><p class="modal-card-title">Rotate API key</p></header>
        <section class="modal-card-body">
          <b-field label="Expires" label-position="on-border">
        ...
        <footer class="modal-card-foot">
          ...
          <b-button type="is-primary" native-type="submit" :loading="rotating">Rotate</b-button>
        </footer>
      </form>
    </b-modal>
    ...
    <b-modal :active.sync="showToken" has-modal-card :can-cancel="false">
      <section class="modal-card">
        <header class="modal-card-head"><p class="modal-card-title">API key</p></header>
        <section class="modal-card-body">
          <b-message type="is-warning" :closable="false">Copy this key now. It will not be shown again.</b-message>
  ...
  methods: {
    ...
    save() {
      if (!this.form.scopes.length) {
        this.$utils.toast('Select at least one permission', 'is-danger');
        return;
      }
      this.saving = true;
    ...
    workspaceName(key) {
      ...
      return org ? org.name : `Organization #${id}`;
    },
    scopeSummary(scopes) { return `${(scopes || []).length} scopes`; },
    ...
    keyStatus(key) {
      if (key.revokedAt || key.revoked_at) return 'Revoked';
      return this.isActive(key) ? 'Active' : 'Expired';
    },
    keyStatusType(key) {
