...
- 公海联系人不再有独立页面（2026-09-17 用户反馈“独立页面体验割裂”）：`frontend/src/views/PoolContacts.vue` 已删除，一级公海与公海分配的联系人改为在“客户”视图（`frontend/src/views/Customers.vue`）里随所选列表就地渲染——该视图先解析被过滤列表的类型（`listDetail`/`currentList` 的 `type`），`pool`/`org_pool_allocation` 走 `GET /api/customer-lists/:id/pool-contacts` 并显示“客户编码/姓名/邮箱/分配部门/状态”只读表格，普通列表仍走 `/api/customers`。旧路径 `/customers/pool-lists/:customerListID`（路由名 `poolContacts`）保留为重定向到 `/customers/customer-lists/:customerListID`；客户列表的“客户数”入口（`CustomerLists.vue` 的 `customerListCustomersRoute`）与公海导入结果的“查看公海联系人”都指向同一路径。数据模型与权限不变：`pool_contacts` 仍是独立存储，普通客户批量操作与导出路径不接触公海数据。
